      ...
      if (isset($_FILES['files'])) {
          ...
          foreach ($_FILES['files']['name'] as $key => $fileName) {
              ...
              if (in_array($fileExt, $allowedExts) && $fileError === 0) {
                  ...
                  echo 'Invalid file type or error with file: ' . $fileName;
              }
          }
          ...
          $query = "INSERT INTO tblmedicalhistory (PatientID, BloodPressure, BloodSugar, Weight, Temperature, MedicalPres, UploadedFiles)
                  VALUES ('$vid', '$bp', '$bs', '$weight', '$temp', '$pres', '$filePaths')";
          
          $result = mysqli_query($con, $query);
  
          if ($result) {
              echo 'Medical history and files uploaded successfully!';
          } else {
...
$(document).ready(function() {
    // Add new file input field when "+" button is clicked
    $('#add-file-btn').click(function() {
        var newFileInput = '<input type="file" name="files[]" class="form-control file-input" multiple>';
        $('#file-inputs').append(newFileInput);
    });
    ...
    $('#medical-history-form').submit(function(e) {
        ...
        $.ajax({
            url: '', // PHP file that handles the form submission
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            success: function(response) {
                // Show response from the server (e.g., success or error messages)
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
     <div class="modal-content">
      <div class="modal-header">
                                                ...
                                                <form id="medical-history-form" method="post" enctype="multipart/form-data">
  ...
  <tr>
    ...
    <td>
        ...
        <button type="button" class="btn btn-info" id="add-file-btn">+ Add More Files</button>
    </td>
</tr>
